...
function draw_status() {
	...
	var status_tab_custom_map_href_exception =
		"<a href='byond://?CustomMapView=1'>View Tactical Map</a>";
	...
	for (var i = 0; i < status_tab_parts.length; i++) {
		if (status_tab_parts[i].trim() == "") {
			...
			status_tab_parts[i] == status_tab_custom_map_href_exception
		) {
...
function draw_verbs(cat) {
	...
	for (var i = 0; i < verbs.length; ++i) {
		...
		if (
			...
			var subCat =
				name.lastIndexOf(".") != -1 ? name.split(".")[1] : null;
...
Byond.subscribeTo("add_debug_tabs", function (ht) {
	href_token = ht;
	addPermanentTab("MC");
});
